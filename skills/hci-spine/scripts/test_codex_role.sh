...
OUTR=$(bash "$ROLE" --role review --cd /tmp --paper draft.md --out /tmp/r.md --dry-run 2>&1); rc=$?
ok "dry-run review → 0" 0 $rc; grok "review dry-run prints command" "$OUTR" "DRY-RUN"

OUTC=$(bash "$ROLE" --role cite_verify --cd /tmp --paper draft.md --bib refs.bib --out /tmp/c.md --dry-run 2>&1)
grok "cite_verify wires --output-schema" "$OUTC" "output-schema"
OUTD=$(bash "$ROLE" --role data_audit --cd /tmp --data results --claims claims.md --out /tmp/d.md --dry-run 2>&1)
grok "data_audit wires --output-schema" "$OUTD" "output-schema"
OUTW=$(bash "$ROLE" --role dual_write --cd /tmp --section "Introduction" --paper draft.md --out /tmp/w.md --dry-run 2>&1)
...
ALLU=$(mktemp); echo '{"citations":[{"key":"x","exists":"unsure","claim_supported":"unsure","note":""}],"summary":"s"}' > "$ALLU"
python3 "$VAL" --role cite_verify "$ALLU" >/dev/null 2>&1; ok "all-unsure cite_verify → 1 (anti-vacuous)" 1 $?
rm -f "$ALLU"
